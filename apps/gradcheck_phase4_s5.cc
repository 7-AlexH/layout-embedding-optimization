// Phase 4 / S5 validation (remove-autodiff plan): the hand-rolled cotan-weight
// stage (Adjoint/CotanStage.{hh,cc}) must reproduce the torch path's forward
// value and stage-input adjoints.
//
// Stage boundary (plan section 4/S5): overlay 3D positions -> per-edge cotan
// weights. Validated COMPOSED with the already-validated S6 (harmonic solve)
// and S7 (distortion) stages. Leaves = overlay positions + per-patch boundary
// UVs. cotans is now DERIVED from the position leaf (not a leaf), so its
// gradient flows back into d_overlay_pos; the position leaf therefore feeds two
// paths (the S5 cotan path + the direct 3D distortion path in S7), and its
// total gradient must match. The non-leaf cotans tensor is retain_grad()'d so
// the intermediate d_cotans is still directly comparable.
//
// Oracle: cotans = torch_cotans(pos_leaf) -> production torch_harmonic_param +
// face loop (torch_face_distortion) -> loss, which must equal the production
// harmonic_distortion_loss BITWISE (torch_cotans on a detached clone of the
// same positions is bitwise-identical, so the production fidelity is intact).
// Hand: cotans_forward(pos) -> harmonic_param_forward -> patch_distortion_forward
// -> loss, with backward distortion_loss_backward -> patch_distortion_backward
// (-> d_inner, d_bnd, d_pos direct) -> harmonic_param_backward (-> d_cotans,
// d_bnd) -> cotans_backward (-> d_pos cotan path).
//
// Forward note: the hand cotans come from Eigen cross/norm/dot, so they differ
// from torch_cotans by O(ulp); the downstream hand solve is therefore NOT
// bitwise-identical to production here (unlike S6, which was fed the torch
// cotans). cotans_fwd_max_abs and inner_solve_max_abs are reported to show the
// propagation stays at ulp scale; the pass gate is the 1e-9/1e-7 adjoint match.
//
// Tolerances: 1e-9 abs / 1e-7 rel (plan section 5). In-process FP constants are
// not trustworthy on Windows (torch_cpu.dll clobbers callee-saved xmm14/15), so
// tolerances are loaded from volatiles right before use and all raw maxima are
// dumped to gradcheck_phase4_s5.txt for an external check.
...
struct ConfigResult
{
    ...
    double cotans_fwd_max_abs = 0.0;   // hand cotans_forward vs torch_cotans (expect ~ulp)
    double inner_solve_max_abs = 0.0;  // hand solve vs production sparse solve (now ~ulp, not 0)
                        ...
                        OverlayMeshData& _omd)
{
    ...
    //=====================================================================
    // stage input: overlay position leaf. cotans is derived from it (oracle)
    // and from its Eigen mirror (hand); the non-leaf cotans tensor is
    // retain_grad()'d so the intermediate d_cotans stays comparable.
    //=====================================================================
    ...
    auto cotans = torch_cotans(pos3_leaf, *_omd.mesh_.get()); // non-leaf, differentiable -> pos
    cotans.retain_grad();
    int64_t const n_edges = cotans.size(0);

    // hand forward of the same stage
    CotanCtx cot_ctx;
    Eigen::VectorXd cotans_e;
    cotans_forward(pos_e, *_omd.mesh_.get(), cot_ctx, cotans_e);

    // forward closeness: hand cotans vs torch cotans
    {
    ...
    // per-overlay-heh uv arrays (torch + plain), boundary UVs are still leaves
    ...
    for (int patch_i = 0; patch_i < n_patches; ++patch_i)
    {
        ...
        auto inner_t = torch_harmonic_param(map_t, _omd.patch_boundary_mask_.value(), inner_vhs_t, bls, cotans, /*sparse*/ true);
        ...
        // hand solve vs production sparse solve (now ~ulp, driven by cotans diff)
        {
    ...
    // S5 backward: fold the accumulated d_cotans into d_overlay_pos (cotan path)
...
int main()
{
    ...
    {
        std::ofstream f("gradcheck_phase4_s5.txt");
    ...
    std::printf("\n%s\n", ok ? "PHASE 4 / S5 GRADCHECK PASSED" : "PHASE 4 / S5 GRADCHECK FAILED");
    return ok ? 0 : 1;
}
